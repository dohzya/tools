...
The `base62` and `hangul` byte/character layouts — the base62 alphabet, the hangul syllable packing, and the registry of short codes for non-default hash tags and `x` values — are not yet fixed by this document. Until a companion registry is published and referenced here, `base62`/`hangul` are not an interoperable wire format between independent implementations: only debug-encoded references are guaranteed to round-trip across implementations. Because the layout is unfixed, an implementation that changes it stays readable by trying each layout it has emitted and discriminating on the compact envelope, whose magic, version byte, and checksum reject a decode at the wrong layout; the current layout must be tried first, so that a corrupt current reference reports its own failure rather than a fallback's.
