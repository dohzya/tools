...
Deno.test("wl: create command creates task", async () => {
  const workspace = createTestWorkspace();
  try {
    await runCli(["init"], workspace);

    const result = await runCli(["create", "Test task"], workspace);
    assertEquals(result.code, 0);
...
Deno.test("wl: create --json outputs task ID", async () => {
  const workspace = createTestWorkspace();
  try {
    await runCli(["init"], workspace);
...
Deno.test("wl: show --json outputs valid JSON", async () => {
  ...
  try {
    ...
    const addResult = await runCli(
      ["create", "Test task", "Test task", "--json"],
      workspace,
    );
    const { id } = JSON.parse(addResult.stdout);
...
Deno.test("wl: list command shows active tasks", async () => {
  ...
  try {
    ...
    await runCli(["create", "Task 1"], workspace);
    await runCli(["create", "Task 2"], workspace);
...
Deno.test("wl: list --json outputs valid JSON", async () => {
  ...
  try {
    ...
    await runCli(["create", "Task"], workspace);
...
Deno.test("wl: import -p PATH imports from path", async () => {
  ...
  try {
    ...
    await runCli(["create", "Task"], workspace1);
...
Deno.test("wl: meta displays task metadata", async () => {
  ...
  try {
    ...
    const addResult = await runCli(
      ["create", "Task with metadata", "--json"],
      workspace,
    );
    const { id } = JSON.parse(addResult.stdout);
...
Deno.test("wl: create --todo creates task with TODOs", async () => {
  const workspace = createTestWorkspace();
  try {
    await runCli(["init"], workspace);
...
Deno.test("wl: scopes delete fails without flags if scope has tasks", async () => {
  ...
  try {
    ...
    await runCli(["create", "Task", "--scope", "lib"], workspace);
