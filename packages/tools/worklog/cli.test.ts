...
Deno.test("worklog import - imports new task from source", async () => {
  ...
  try {
    ...
    assertEquals(destTasks.tasks[0].name, "Source task");
...
Deno.test("worklog import - removes source tasks with --rm", async () => {
  ...
  try {
    ...
    Deno.chdir(tempDirSource);
    await main(["init"]);
    await main(["create", "Task to remove"]);
...
Deno.test("worklog import - warns when entry older than checkpoint", async () => {
  ...
  try {
    ...
    Deno.chdir(tempDirDest);
    await main(["init"]);
    await main(["create", "Task with checkpoint"]);
    const listDest = await captureOutput(() => main(["list", "--json"]));
    const { tasks: destTasks } = JSON.parse(listDest);
    const taskId = destTasks[0].id;
    await main(["trace", taskId, "Entry 1"]);
...
Deno.test("worklog import - handles external worktree with --scope-to-tag", async () => {
  ...
  try {
    ...
    await main(["init"]);
    await main(["create", "External task"]);
...
Deno.test("worklog purge - preserves tasks with uncheckpointed entries", async () => {
  ...
  try {
    ...
    await main(["init"]);
    await main(["create", "Old task with changes"]);
...
Deno.test("worklog - JSON output for list command", async () => {
  ...
  try {
    ...
    await main(["init"]);
    await main(["create", "Task 1", "--started"]);
    await main(["create", "Task 2", "--started"]);

    const output = await captureOutput(() => main(["list", "--json"]));
    const result = JSON.parse(output);

    assertEquals(result.tasks.length, 2);
    const names = result.tasks.map((t: { name: string }) => t.name).sort();
    assertEquals(names, ["Task 1", "Task 2"]);
    assert(
...
Deno.test("worklog - JSON output for show command", async () => {
  ...
  try {
    ...
    await main(["init"]);
    await main(["create", "Test task", "--started"]);
...
// Integration tests for wl create with timestamp
Deno.test("worklog create - accepts custom timestamp in ISO format", async () => {
  const tempDir = await Deno.makeTempDir();
  const originalCwd = Deno.cwd();
  try {
    Deno.chdir(tempDir);
...
Deno.test("worklog create - accepts -t as alias for --timestamp", async () => {
  const tempDir = await Deno.makeTempDir();
  const originalCwd = Deno.cwd();
  try {
    Deno.chdir(tempDir);
    ...
    const customTimestamp = "2024-11-20T09:00:00+01:00";
    await main(["create", "Task with -t", "-t", customTimestamp]);
...
Deno.test("worklog create - accepts flexible timestamp T format", async () => {
  const tempDir = await Deno.makeTempDir();
  const originalCwd = Deno.cwd();
  try {
    Deno.chdir(tempDir);
    ...
    await main(["create", "Task with T format", "-t", "T16:45"]);
...
Deno.test("worklog create - adds local timezone when missing", async () => {
  const tempDir = await Deno.makeTempDir();
  const originalCwd = Deno.cwd();
  try {
    Deno.chdir(tempDir);
...
Deno.test("worklog create - rejects invalid timestamp format", async () => {
  const tempDir = await Deno.makeTempDir();
  const originalCwd = Deno.cwd();
  let _exitCode = 0;
  let errorOutput = "";
...
Deno.test("show - backward compat: parses old ### Insights header as learnings", async () => {
  const tempDir = await Deno.makeTempDir();
  const originalCwd = Deno.cwd();
  try {
    Deno.chdir(tempDir);
    ...
    await main(["create", "--started", "Task with old insights header"]);
    const _ls = await captureOutput(() => main(["list", "--all", "--json"]));
    const id = JSON.parse(_ls).tasks[0].id;

    await main(["trace", id, "Some work"]);
    await main(["checkpoint", id, "Changes made", "Old insights content"]);

    // Replace ### Learnings with old ### Insights in the task file
    const taskPath = `.worklog/tasks/${id}.md`;
    let content = await Deno.readTextFile(taskPath);
    content = content.replace("### Learnings", "### Insights");
    await Deno.writeTextFile(taskPath, content);

    const showOutput = await captureOutput(() => main(["show", id]));

    assertStringIncludes(showOutput, "Changes made");
    assertStringIncludes(showOutput, "Old insights content");
  } finally {
    Deno.chdir(originalCwd);
    await Deno.remove(tempDir, { recursive: true });
  }
...
Deno.test("regression - wl trace with explicit taskId still works", async () => {
  ...
  try {
    Deno.chdir(tempDir);
    await main(["init"]);
    await main(["create", "Regression test task"]);
...
Deno.test("regression - wl done with explicit taskId still works", async () => {
  ...
  try {
    Deno.chdir(tempDir);
    await main(["init"]);
    await main(["create", "Done regression task"]);
...
Deno.test("regression - wl checkpoint with explicit taskId still works", async () => {
  ...
  try {
    Deno.chdir(tempDir);
    await main(["init"]);
    await main(["create", "Checkpoint regression task"]);
