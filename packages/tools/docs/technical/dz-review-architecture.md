...
2. **Fallback — resolve remaining stored entries against the current document.** For items that did not fast-match, take the remaining unconsumed entries for this file, rank their stored MRFIs by `RankReferenceCandidatesUseCase` (implements docs/specs/mrfi.md's `rank(target, candidates)`: compares `target` against each candidate MRFI via `compare(A, B)` and orders by `(verdict class, similarity)`), then resolve each ranked candidate's MRFI against the current document in order. Here `target` is `item.raw` — free witness text, not an MRFI reference — so it never parses as a locator and every candidate compares `invalid`; ranking degrades to the candidates' stored order, same as before this use case was implemented. Real ranking benefit is a seam for a future caller with an actual target reference. Adopt the first resolution whose status is `exact` or `confident` and whose resolved range overlaps the item's current lines. On adoption, regenerate the entry's `{range, mrfi}` directly from the range already resolved in this step — deliberately not via `refreshReference`, which would resolve the same reference a second time internally.
...
- `RankReferenceCandidatesUseCase` ranks by MRFI comparison (see pass 2 above), but this caller has no MRFI-shaped target to rank against — only free witness text — so with more than one plausible candidate entry for a file, the fallback still tries them in stored order, not closest-match-first. Wiring a real target reference through here is unimplemented future work.
