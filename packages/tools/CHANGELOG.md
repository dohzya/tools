...
- **worklog:** Flexible timestamp support for `wl trace` command via `--timestamp` / `-t` flag
  - Format: `[YYYY-MM-DD]THH:mm[:SS][<tz>]`
  - Automatically fills missing components: date (today), seconds (:00), timezone (local)
